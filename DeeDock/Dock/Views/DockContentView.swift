...
struct DockContentView: View {
    ...
    private var slots: [DockRenderSlot] {
        DockRenderSlot.slots(
            entries: entries ?? items.map(DockRenderSlot.app),
            proposal: interaction.dragProposal
        )
    }
    private var sizes: [CGFloat] {
        layout.sizes(
            pointerAlong: interaction.dragActive
                ? nil
                : interaction.pointer.map {
    ...
    private func shifted(_ rect: CGRect, by offset: CGFloat) -> CGRect {
        rect.offsetBy(
            dx: layout.edge.isVertical ? 0 : offset,
    ...
    var body: some View {
            ...
            ScrollViewReader { proxy in
                ScrollView(edge.isVertical ? .vertical : .horizontal) {
                    DockSurfaceView(
                        ...
                        iconFrameChanged: { id, rect in
                            guard interaction.layout.edge == edge else {
                                return
                            }
                    ...
                    .onGeometryChange(for: CGFloat.self) { geometry in
                        edge.along(
                            geometry.frame(in: .named("dockViewport")).origin
                        )
                    } action: { value in
                        guard interaction.layout.edge == edge else { return }
                        guard
                            scrollOffset != value
                                || interaction.scrollOffset != value
                        else { return }
                        scrollOffset = value
                        interaction.scrollOffset = value
                ...
                .onChange(of: interaction.scrollRequest) { previous, current in
                    let offset = min(
                        ...
                        max(0, layout.canvasLength - layout.viewportLength)
                    )
                    if edge.isVertical {
                ...
                .onAppear {
                    ...
                    if keyboardFocus, let selectedTarget {
                        proxy.scrollTo(selectedTarget.hitID, anchor: .center)
                    }
                }
        ...
        .onChange(of: interaction.pointer == nil) { _, outside in
            if outside { hoveredID = nil }
        }
    ...
    #Preview("Drag: empty dock with incoming apps") {
        DockPreviewContent(
            ...
            dragProposal: DockDragProposal(
                pins: Array(DockPreviewData.items.prefix(2)).map {
                    .application($0.reference)
                },
                index: 0
            ),
            dragMessage: .dragPinHere
