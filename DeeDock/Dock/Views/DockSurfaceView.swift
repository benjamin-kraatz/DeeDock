    ...
    var body: some View {
        ZStack(alignment: .topLeading) {
                ...
                if index < sizes.count {
                    ...
                    if let item = slot.item {
                        ...
                        RoundedRectangle(cornerRadius: 10)
                            ...
                            .position(x: frame.midX, y: frame.midY - 6)
                            .accessibilityHidden(true)
            ...
            if let id = hoveredID ?? (keyboardFocus ? selectedID : nil),
               let index = slots.firstIndex(where: { $0.item?.id == id }), index < centers.count, showsLabel {
        ...
        .animation(reduceMotion ? nil : .easeOut(duration: 0.15), value: slots.map(\.id))
