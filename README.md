...
- Optional **Peek history** saves recognized text from peeks locally after opt-in under **Settings → Features → Window Peek**. Search recent text, delete individual entries, or clear the history. Images are not saved. See [Peek history](docs/PEEK-HISTORY.md) for privacy, retention, and OCR limits.
