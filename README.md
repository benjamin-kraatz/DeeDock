...
- Choose **Watch this** on a Window Peek card, or press W in keyboard Peek, to monitor a selected region for stable visual change or an exact completion phrase. A persistent panel keeps Stop available. Results are local visual evidence, with optional sound and indeterminate progress. See [Watch a window](docs/WINDOW-WATCH.md) for controls, capture limits, and pending native acceptance.
