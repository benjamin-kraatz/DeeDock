...
Settings → Features → Clipboard Museum turns collecting on. It is off by default, and the
menu-bar item and app menu gain **Clipboard Museum…**. The window groups exhibits by day in a
sidebar of placards (catalog number, time, provenance). A search field and a Gallery filter
(All, Text, Links, Images, Files, Vault) narrow the list. The detail hangs one piece in a frame
on a lit wall, with a wall label listing Acquired, Provenance, Dimensions, Condition, and a
detected secret's kind. From the detail, a person can copy an exhibit back to the clipboard,
redact it, or remove it. Delete removes the selected exhibit. Clear Collection asks first.

Collecting uses public `NSPasteboard` API only. AppKit has no pasteboard change notification,
so a 0.75 s timer (50% tolerance) compares `changeCount` while collecting is on. It stops when
collecting is off, when storage needs a reset, and on quit. Clipboard contents present when
collecting starts are not collected. One representation is read per change: file URLs, then a
PNG/TIFF image, then text or a single http(s) link. Provenance is the frontmost app at the
time of the change. macOS does not report the actual source. Copies back from the museum
are acknowledged and not collected again. A repeated consecutive copy is skipped by an
in-memory hash that is never written.

Privacy:

- Items marked with the nspasteboard.org Concealed, Transient, or AutoGenerated types (and
  1Password's marker) are never read.
- Local detection flags PEM private keys, vendor token prefixes (AWS, GitHub, Slack, Stripe,
  `sk-`, Google, GitLab), JWTs, Luhn-valid card numbers, `password=`-style assignments, and
  unbroken mixed-class generated-looking strings. With **Redact secrets automatically** on
  (default), flagged text is stored only as a veil. The only hint kept is a vendor prefix or a
  card's last four digits. With it off, exhibits are marked "Looks sensitive". Turning it on
  later veils them.
- Redaction seals the content (text, image, recognized text) with AES-GCM into `Vault/*.sealed`
  and clears every plain field, including curator wording. See the second pass below.
- Storage is `Application Support/<bundle id>/ClipboardMuseum` (`collection.json` plus
  `Images/*.png`). The folder is 0700, files are 0600, and the folder is excluded from backups.
  There is no sync path.
- An unreadable collection freezes edits and collecting until Reset Collection.

Limits: 200 exhibits (oldest retire with their images), 20,000 characters per text, 50 files
per copy, 32 MiB image input, 40 million pixels, stored images downsampled to 2,048 px PNG,
16 MiB collection file. Images are encoded off the main actor in copy order.

API limitations: the first content read after collecting starts can show macOS's clipboard
privacy prompt. When access is set to always deny, nothing is read and Settings says so.
Detection is heuristic. It can miss secrets and can flag random-looking non-secrets.

Validation: `xcodebuild -project DeeDock.xcodeproj -scheme DeeDock -configuration Debug
-destination 'platform=macOS' -derivedDataPath /tmp/deedock-dee41-build CODE_SIGNING_ALLOWED=NO
build` succeeded. `DeeDockTests/ClipboardMuseumTests.swift` covers detection, hints, the
off-by-default rule, redaction on disk, late redaction, remove/clear numbering, duplicates,
retention, truncation, corrupt storage, concealed items, and link parsing. Tests, previews,
and automated visual checks were not run. The app was not launched.

Pending native acceptance:

- Turn collecting on and copy from Safari, Notes, Finder, Preview, Screenshot, and a password
  manager. Confirm the macOS prompt, the always-deny notice, concealed items being skipped,
  provenance, and image fidelity.
- Copy exhibits back and paste into other apps: text, links, several files, images.
- Redact, remove, clear, relaunch, and corrupt `collection.json` by hand. Confirm image files
  are deleted and the folder permissions and backup exclusion hold.
- Check English and German copy, VoiceOver, keyboard selection and Delete, Dark Mode, Reduce
  Transparency, window focus return on close, Spaces, and sleep/wake with collecting on.

### Second pass: rename, reveal, Vision, curator, export, slideshow

- **Rename.** You can rename an exhibit by double-clicking the placard title, clicking the pencil
  that appears on hover, pressing Return on the selected row, or choosing Rename from the context
  menu. Return or clicking away saves, and Escape cancels. An empty field returns the exhibit to
  its automatic name, which the field shows as its placeholder. Title precedence is custom name,
  then veil hint, then curator title, then content. Custom names are searchable.
- **Reveal and Not a Secret.** Sealed exhibits offer Reveal. It uses LocalAuthentication
  `.deviceOwnerAuthentication` (Touch ID, Apple Watch, or the login password). Revealed content
  lives only in the detail view's state. Selecting another exhibit, Hide, or closing the window
  drops it. Revealed content can be copied (marked `org.nspasteboard.ConcealedType`) and saved.
  **Not a Secret** authenticates, restores the content permanently, and stores a SHA-256 of that
  text so it is not flagged again (max 500 digests, only for text the person declared harmless).
  **Shred** deletes the sealed file. Redaction then becomes permanent.
...
- **Vision.** After an image is stored, `ClassifyImageRequest` (labels with confidence at least
  ...
  checked for secrets. A secret read from an image seals the image and its text. Labels are
  Vision's English taxonomy identifiers.
- **Curator (Foundation Models).** When `SystemLanguageModel.default` is available with guided
  generation, each new non-sensitive exhibit gets a two-to-six-word title and a one-sentence
  note. Inputs are text, link, file names, or image labels plus OCR text. It uses a fresh session,
  greedy sampling, 120 tokens, no tools, and treats content as untrusted. It runs one request at
  a time after capture and is cancelled on stop. Flagged or redacted exhibits are never sent,
  and the store refuses curator wording for them. The Settings toggle and all curator UI are
  absent when the model is unavailable. Titles and notes are searchable, so screenshots can be
  found by what they show.
- **Save to Files.** Text and links save as .txt or .md, images as .png, and files as copies into
  a chosen folder (numbered on name collision) or a .md list. Markdown has a title heading, a
  catalog line, the curator note, and the content. Nothing is written without a native panel.
- **Slideshow.** The toolbar button (Control-Command-F) plays visible, non-redacted exhibits
  starting at the selection. It opens a window that enters native full screen and closes when
  full screen ends. Slides cross-dissolve with a directional drift and settle-in scale using a
  1.2 s smooth animation, and each piece drifts forward slowly while it hangs (7 s). Reduce
  Motion keeps only a 0.6 s dissolve. Arrow keys step, Space pauses, and Escape leaves. Controls
  fade after 2.5 s without pointer movement.
- **Settings.** With collecting off, the redaction and curator switches are disabled. Open and
  Clear stay available for stored exhibits.

Second-pass validation: the same Debug `xcodebuild` command succeeded. The tests file was
extended (reveal, Not a Secret, shred, secrets in images, Vision search, rename, curator guard,
Markdown). Tests, previews, and the app were not run.

Pending native acceptance adds: Touch ID and password prompts and cancel; a Keychain prompt on
first seal; Vision on screenshots, photos, and receipts in English and German; curator output on
