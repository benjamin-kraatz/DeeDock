...
A scheduled offer also shows a dismissible callout on the main display, a badge on the menu-bar extra, a badge on Settings → General and Software Update, and a soft indigo-to-coral pip on DDock glass (and on DDock’s own tile when that tile is present). Reduce Motion keeps the pip static. Dismissing the callout, opening the Update window, installing, or skipping clears the badges and pip. They stay quiet for that offer until a later identity or the next launch. Sparkle’s automatic-check interval is unchanged.

**Install & relaunch when idle** is a DDock preference, off by default. Settings and the ready-to-install Update window share it. The ready-window switch is not shown during onboarding. When a download is ready, DDock makes one install attempt after a quiet period with no pin drag, Focus panel, Update window, file picker, dock popover, or menu tracking. If idle never arrives, Sparkle’s install-on-quit path remains. Sparkle still owns download, verify, and install.
