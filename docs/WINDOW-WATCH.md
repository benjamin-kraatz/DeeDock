...
labeled sliders with the keyboard. The orange outline shows the exact region after boundary clamping.
...
**Dismiss**, Escape, and the panel's close button do not activate the source application. Dismiss
...
continue monitoring. The running application instance and launch date guard against process restart.
...
| State | Behavior |
| --- | --- |
| Hidden app, minimized window, other Space, disconnected source display | No hidden-window guarantee. Offscreen metadata pauses capture, labels the preview stale, and retries after three seconds. |
...
| Stop, dismiss, app shutdown | Cancel tasks and remove observers. No periodic capture remains. An OS request already in flight may finish; cancelled results are ignored. |
